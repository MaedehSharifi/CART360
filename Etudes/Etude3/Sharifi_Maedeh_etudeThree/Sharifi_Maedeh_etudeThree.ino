...
void chooseMode(){
  ...
  {
    delay(400);
    if (mode < 4) mode++; //mode is incremented
    else mode = 0; //mode is reset to 0 if it is higher than 4
  }
}
...
void live()
{
   //getiing samples from photocell frequencies
   offsetFrequency = getPhotoFrequency();
   //using Running Average algorithm to find a better photocell frequency (with less noise)
   averageOffsetFreq=getRunningAverage();
   
   
   // read in the value from button press
    testNote = analogRead(NOTE_IN_PIN);
    // output sound: photocell + keyboard frequencies
    tone(BUZZER_PIN, testNote+averageOffsetFreq,100);  
...
{
  // to handle a little bit of noise (8 is the threshold)
  int theAdjuster = 8;
  ...
  if (countNotes < MAX_NOTES)
  {
    ...
    //if the generated frequency is higher than the threshold and the time passed in less than 5 sec.
    if (testNote > theAdjuster && (timePassed < MAX_PRESS_TIME)) {
      ...
      */
      ...
      { 
        ...
        updateTimer();
         /*** FUNCTION TO IMPLEMENT ***/
        //sampling the photocell frequencies
        offsetFrequency = getPhotoFrequency();
         /*** FUNCTION TO IMPLEMENT ***/
        //playing the note
        playCurrentNote();
         /*** FUNCTION TO IMPLEMENT ***/
        //find a better value of the photocell frequency (with less noise)
        averageOffsetFreq = getRunningAverage();
       }
    }
     /*** STATE B::: *******************************************
     ...
     */
    //if the key is pressed and the time passed has surpassed the max value (5 sec)
    ...
    //if the key is no longer pressed 
       ...
       /*** FUNCTION TO IMPLEMENT ***/
      //recording the keyboard frequency and its duration
...
{
  //IMPLEMENT
  activeNoteButton = true; //key has been pressed
  activeFrequency = testNote; //frequency of the keyboard
  startTime=millis(); //storing time of keypress
  timePassed=0; // Time passed after start time

  //we can initialise running average array to 0
...
void updateTimer()
{
 ...
 //IMPLEMENT
 timePassed=millis()-startTime; // Time passed after start time
...
{
    //IMPLEMENT
    tone(BUZZER_PIN, activeFrequency+averageOffsetFreq,100); //could be offsetFrequency
    delay(100);// wait for tone to finish
...
{
 //IMPLEMENT
  // record note (keyboard + photocell frequencies)
  notes[countNotes] = activeFrequency+averageOffsetFreq; 
  //if time is longer than the max value (5 sec), keep the max value
  if (timePassed > MAX_PRESS_TIME) timePassed=MAX_PRESS_TIME; 
  // record the duration of the note
  durations[countNotes] =  timePassed; 
  countNotes++; //increment countNotes
  //change boolean to false since the button is no longer pressed
...
{
  //IMPLEMENT
  int invScale=3; //scale value = 1/invScale 
  // (photocell frequency - threshold (in order to obtain no frequency when it is dark)) divided by invScale
  return( (analogRead(PHOTO_PIN)-45)/invScale);
}

/******************GETRUNNINGAVERAGE(): IMPLEMENT *********************************
 * INSTRUCTIONS:
 * this function should compute the running average of the phot0cell value (returned from the 
...
int getRunningAverage()
{
 //IMPLEMENT
 int currentSum= 0;
 //store photocell sample frequencies in array
 runningAverageBuffer[nextCount]=offsetFrequency;
 nextCount++; //increment nextCount
 
 //if the array is full, we return to the first position
 if (nextCount >= RUNNING_SAMPLES) nextCount=0;
 
 //calculating and returning the average frequency
 for(int i=0; i< RUNNING_SAMPLES; i++){
...
{
  analogWrite(LED_PIN_R, 0);   // Turn on the LED -R
  //col = intensity of the LED from 0 to 255
...
void playWithDuration()
{
  ...
  for(int i=0; i<countNotes;i++)
  {
    //calling a function to light up the LED with the appropriate intensity
    colorLED(durations[i]*255/MAX_PRESS_TIME);
    //play with the appropriate duration
